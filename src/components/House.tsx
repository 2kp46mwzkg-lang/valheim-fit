...
function PanelHeader({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="flex items-start gap-3">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#1a241f] text-xl">{icon}</div>
      <div>
        <h2 className="font-display text-base font-bold text-[#f2d16b]">{title}</h2>
        <p className="text-[11px] leading-relaxed text-[#a8b3a5]">{desc}</p>
      </div>
    </div>
  );
}
...
export default function House({
  ...
  // retour d'une séance → on arrive par le rivage
  useEffect(() => {
    if (arrivalToken > 0) setLoc('rive');
  }, [arrivalToken]);
  ...
  const spot = CAMP_SPOTS.find((s) => s.id === loc)!;

  /* ============================ RIVAGE ============================ */
  const rivePanel = (
    <div className="space-y-3">
      <PanelHeader
        icon={<Waves className="text-[#7fa8c9]" size={22} />}
        title="Le Rivage"
        desc={spot.desc}
      />
      <button onClick={onGoUpload} className="btn-wood flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3.5 font-bold">
        <Flame size={17} /> Je reviens de l’effort — récolter
      </button>
      {last ? (
        <div className="card-viking p-3.5">
          <div className="text-[10px] font-bold uppercase tracking-wider text-[#7d8a80]">Dernière arrivée</div>
        ...
        <div className="rounded-2xl border border-dashed border-[#3f5548] p-4 text-center text-[11px] leading-relaxed text-[#7d8a80]">
          ...
          Le sable est vierge. Fais un premier effort puis reviens ici pour charger ta séance
          (fichier Garmin <b>.fit / .zip</b> ou saisie manuelle).
        </div>
      )}
    </div>
  );
  ...
  const runePanel = (
    <div className="space-y-3">
      <PanelHeader
        icon={<span className="font-display text-[#f2d16b]">ᚱ</span>}
        title="Pierre des sagas"
        desc={spot.desc}
      />
      <div className="card-viking p-4">
        ...
        {nextZone ? (
          <>
            <div className="bar-track mt-3">
              <div
                ...
                style={{ width: `${Math.min(100, ((state.totalPoints - prevAt) / (nextZone.at - prevAt)) * 100)}%` }}
              />
            </div>
          ...
          <p className="mt-3 text-[11px] font-bold text-[#7fbf7f]">🗺️ Toute la prairie est révélée.</p>
        )}
      </div>
        ...
        <div key={q.id} className="card-viking p-3.5">
          <div className="flex items-center justify-between">
            <b className="flex items-center gap-2 text-sm text-[#ece6d6]">
              ...
              {q.completed > 0 && <span className="rounded-full bg-[#7fbf7f]/15 px-1.5 py-0.5 text-[10px] font-bold text-[#7fbf7f]">×{q.completed}</span>}
            </b>
      ...
      <button onClick={onGoQuests} className="btn-ghost flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold">
        <ScrollText size={15} /> Ouvrir le livre des quêtes
      </button>
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      {!built.includes('abri') && (
        <div className="rounded-xl bg-[#e8955a]/10 px-3 py-2 text-[11px] text-[#e8955a]">
          Une simple souche pour l’instant. Construis l’<b>Abri</b> pour monter un vrai établi.
        </div>
      )}
      {CRAFTS.map((cr) => {
    ...
    <div className="space-y-3">
      ...
      {nextBuild ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-[#f2d16b]/40 bg-gradient-to-r from-[#f2d16b]/15 to-[#7fbf7f]/10 p-4">
          ...
          <p className="mt-1 text-xs text-[#a8b3a5]">Fabrique la Tenue en cuir à l’établi pour ouvrir la Forêt Noire.</p>
        </div>
      )}
        ...
        {showSave && (
          <div className="card-viking space-y-2 p-4">
            ...
            <p className="text-[11px] leading-relaxed text-[#7d8a80]">
              Ta progression est sauvegardée dans ce navigateur. Exporte-la avant de changer d’appareil.
            </p>
            {!confirmReset ? (
  ...
  const feuPanel = (
    <div className="space-y-3">
      <PanelHeader
        icon={<Flame className="text-[#e8955a]" size={22} />}
        title="Feu de camp"
        desc={spot.desc}
      />
      {!feuBuilt ? (
        <div className="card-viking space-y-3 p-4">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#1a241f]"><Lock className="text-[#5a6a60]" size={20} /></div>
            <p className="text-xs leading-relaxed text-[#a8b3a5]">
              Quelques pierres froides attendent. Le feu de camp est ta toute première construction : il permet de cuisiner.
            </p>
          </div>
          <CostLine cost={BUILDINGS[0].cost} inv={state.inventory} />
          <button
            disabled={!canBuild('feu')}
            onClick={() => doBuild('feu')}
          ...
          >
            <Flame size={15} /> Allumer le feu de camp
          </button>
        </div>
      ) : (
        <>
          <div className="card-viking space-y-3 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-[11px] text-[#a8b3a5]">×1.2 sur les points de ta prochaine sortie. Fabriqué à l’établi (3 🫐 + 1 🍖).</div>
              </div>
              <span className="rounded-full bg-[#1a241f] px-2.5 py-1 text-sm font-bold text-[#ece6d6]">🍲 ×{state.inventory.repas ?? 0}</span>
            </div>
            {state.buff ? (
              <div className="pop rounded-xl border border-[#f2d16b]/40 bg-[#f2d16b]/10 px-3 py-2.5 text-center text-xs font-bold text-[#f2d16b]">
                Déjà actif : ×{state.buff.mult} sur ta prochaine séance
              </div>
            ) : (
              <button
                disabled={(state.inventory.repas ?? 0) < 1}
                onClick={() => eatMeal()}
          ...
          <div className="card-viking grid grid-cols-2 divide-x divide-[#3f5548] p-4 text-center">
            <div>
              <div className="font-display text-xl font-bold text-[#f2d16b]">×{lootMultiplierForComfort(comfort).toFixed(2)}</div>
              <div className="text-[10px] uppercase tracking-wider text-[#a8b3a5]">butin du foyer (confort {comfort}/5)</div>
            </div>
            <div>
              <div className="font-display text-xl font-bold text-[#ece6d6]">{state.sessions.length}</div>
              <div className="text-[10px] uppercase tracking-wider text-[#a8b3a5]">sagas contées au feu</div>
            </div>
          </div>
          <p className="px-1 text-center text-[11px] italic leading-relaxed text-[#7d8a80]">
            « Celui qui construit son foyer avant l’hiver mange chaud à la première neige. »
          </p>
        </>
      )}
    </div>
            ...
            return (
              <button
                ...
                className={`chip-loc flex shrink-0 items-center gap-1.5 rounded-full border px-3.5 py-2 text-xs font-bold ${
                  active
                    ? 'border-[#f2d16b] bg-[#f2d16b] text-[#221806] shadow-[0_6px_18px_-6px_rgba(242,209,107,.6)]'
                    : unlocked
                      ? 'border-[#3f5548] bg-[#24332b] text-[#ece6d6]'
                      : 'border-[#3f5548]/60 bg-[#1c2620] text-[#5a6a60] border-opacity-60'
                }`}
              >
